...
## Agenti e swarm — usa SEMPRE Ruflo

Quando spawni agenti o esegui task paralleli:
1. Carica schema prima: `ToolSearch("select:mcp__ruflo__swarm_init,mcp__ruflo__terminal_execute,mcp__ruflo__agent_spawn")`
2. Usa `mcp__ruflo__swarm_init` + `mcp__ruflo__agent_spawn` — MAI `caveman:*` o subagent_type generici per task multi-file
3. Per comandi shell su più file → `mcp__ruflo__terminal_execute`
4. "usa ruflo" detto dall'utente = hard requirement, non suggerimento
